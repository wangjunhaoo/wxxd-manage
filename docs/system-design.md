...
桌面端启动后在 Rust 侧启动一个本地主控 HTTP API，供外部选品系统或自动化脚本调用。当前实现固定监听 `127.0.0.1:17890`，只面向本机进程，不默认开放局域网访问。

认证规则：

- 业务接口必须携带 `x-wx-xd-api-key`，或使用 `Authorization: Bearer <API Key>`。
- API Key 通过桌面端生成或重置，后端只保存 SHA-256 摘要和尾号提示。
- 未生成 API Key 时业务接口返回 `LOCAL_API_KEY_NOT_CONFIGURED`，不会降级为免认证。
- HTTP API 不记录 API Key，不在文档样例中写固定 Key。
- 外部 API 审计写入 `external_api_logs`，只记录方法、路径、状态码、耗时、错误码、脱敏请求摘要和脱敏响应摘要。
- 铺货和改价创建的请求摘要只允许包含 `request_id`、商品数量、店铺组数量和店铺数量；不得保存完整商品 payload。
- 采购任务接口只允许查询非敏采购字段、回填供应商物流和标记供应商异常；不得通过本地 HTTP API 暴露收件人姓名、手机号、地址，也不得自动向供应商平台下单。
- 采购任务接口审计只允许记录任务 ID、异常类型、是否有物流单号/备注等脱敏摘要，不保存完整物流请求体。
- 供应商 agent 安全桥只允许导出非敏采购字段，并只写回 `shipment`、`issue`、`mapping` 三类结果；包含收件信息、密钥、Cookie、供应商平台凭证或未知字段的记录必须拒绝。
- 售后拒绝原因和处理接口只允许记录店铺 ID、售后 ID、动作、原因类型、适用场景、是否带备注等脱敏摘要；不得记录完整售后详情、凭证、收件信息或微信 access_token。
- 售后责任归因和订单利润接口只允许记录售后 ID、订单 ID、责任方、调整类型、金额和是否有备注；不得记录完整售后详情、凭证、收件信息或供应商平台凭证。
- 本地凭证资料包接口只允许记录目标类型、目标是否存在、凭证类型、状态以及是否有说明/文件/来源链接；状态更新接口只允许记录凭证 ID 是否存在和目标状态；导出接口只允许记录筛选条件、格式和导出条数，不得在审计日志里记录完整说明、本地路径、来源链接或图片内容。
- 后续供应商下单 agent/skill 优先复用 `scripts/wx_xd_local_api.py` 或同等 HTTP 协议，只通过本地主控 API 回填结果，不直接读写 SQLite。
...
| --- | --- | --- |
| `GET` | `/health` | 本地主控健康检查 |
| `GET` | `/api/shop-groups` | `list_shop_groups` |
| `GET` | `/api/shops` | `list_shops` |
| `POST` | `/api/publish-jobs` | `create_external_publish_job` |
| `GET` | `/api/publish-jobs/{task_id}` | `get_publish_job` |
| `POST` | `/api/price-update-jobs` | `create_price_update_job` |
| `GET` | `/api/price-update-jobs/{task_id}` | `get_price_update_job` |
| `GET` | `/api/task-runs` | `list_task_runs` |
| `GET` | `/api/notifications` | `list_notifications` |
| `POST` | `/api/notifications/{notification_id}/read` | `mark_notification_read` |
| `GET` | `/api/purchase-tasks` | `list_purchase_tasks` |
| `POST` | `/api/purchase-tasks/{purchase_task_id}/mapping` | `resolve_purchase_task_mapping` |
| `POST` | `/api/purchase-tasks/{purchase_task_id}/shipment` | `record_purchase_task_shipment` |
| `POST` | `/api/purchase-tasks/{purchase_task_id}/issue` | `mark_purchase_task_issue` |
| `GET` | `/api/aftersales` | `list_aftersales` |
| `GET` | `/api/aftersale-reject-reasons` | `list_aftersale_reject_reasons` |
| `POST` | `/api/aftersale-reject-reasons/sync` | `sync_aftersale_reject_reasons` |
| `POST` | `/api/aftersales/{aftersale_id}/accept` | `accept_aftersale` |
| `POST` | `/api/aftersales/{aftersale_id}/reject` | `reject_aftersale` |
| `POST` | `/api/aftersales/{aftersale_id}/responsibility` | `record_aftersale_responsibility` |
| `GET` | `/api/aftersale-evidence` | `list_aftersale_evidence` |
| `POST` | `/api/aftersale-evidence` | `record_aftersale_evidence` |
| `POST` | `/api/aftersale-evidence/{evidence_id}/status` | `update_aftersale_evidence_status` |
| `POST` | `/api/aftersale-evidence/export` | `export_aftersale_evidence` |
| `GET` | `/api/supplier-aftersale-followups` | `list_supplier_aftersale_followups` |
| `POST` | `/api/supplier-aftersale-followups` | `record_supplier_aftersale_followup` |
| `GET` | `/api/guarantee-orders` | `list_guarantee_orders` |
| `POST` | `/api/guarantee-orders/{guarantee_order_id}/followup` | `record_guarantee_followup` |
| `GET` | `/api/order-profits` | `list_order_profit_summaries` |
| `POST` | `/api/order-profits/{order_id}/adjustments` | `record_order_profit_adjustment` |
| `GET` | `/api/inventory-risks` | `list_inventory_risks` |
| `GET` | `/api/product-sales-analysis` | `list_product_sales_analysis` |
| `GET` | `/api/delivery-settings` | `get_delivery_settings` |
| `POST` | `/api/delivery-settings/auto-send` | `set_auto_send_delivery` |
| `GET` | `/api/delivery-companies` | `list_delivery_companies` |
| `POST` | `/api/delivery-companies/sync` | `sync_delivery_companies` |
| `GET` | `/api/delivery-shipments` | `list_delivery_shipments` |
| `POST` | `/api/delivery-shipments` | `record_order_shipment` |
| `POST` | `/api/delivery-shipments/{shipment_id}/retry` | `retry_delivery_shipment` |
| `POST` | `/api/runners/order-sync` | `run_order_sync_once` |
| `POST` | `/api/runners/order-detail-sync` | `run_order_detail_sync_once` |
| `POST` | `/api/runners/aftersale-sync` | `run_aftersale_sync_once` |
| `POST` | `/api/runners/purchase-task-generation` | `run_purchase_task_generation_once` |
| `POST` | `/api/runners/delivery-submit` | `run_delivery_submission_once` |
| `POST` | `/api/runners/publish-precheck` | `run_publish_tasks_once` |
| `POST` | `/api/runners/publish-attribute-fill` | `run_publish_attribute_fill_once` |
| `POST` | `/api/runners/publish-ai-attributes` | `run_publish_ai_attribute_suggestions_once` |
| `POST` | `/api/runners/publish-category-precheck` | `run_publish_category_prechecks_once` |
| `POST` | `/api/runners/publish-assets` | `run_publish_asset_uploads_once` |
| `POST` | `/api/runners/publish-submit` | `run_publish_submits_once` |
| `POST` | `/api/runners/publish-status-sync` | `run_publish_status_sync_once` |
| `POST` | `/api/runners/publish-listing` | `run_publish_listing_once` |
| `POST` | `/api/runners/price-precheck` | `run_price_update_precheck_once` |
| `POST` | `/api/runners/price-submit` | `run_price_update_submit_once` |
| `POST` | `/api/runners/price-confirm` | `run_price_update_confirm_once` |
| `POST` | `/api/runners/inventory-risk-scan` | `run_inventory_risk_scan_once` |
| `POST` | `/api/runners/guarantee-sync` | `run_guarantee_sync_once` |
| `POST` | `/api/runners/operations` | `run_operational_automation_once` |

零依赖本地调用器 `scripts/wx_xd_local_api.py` 覆盖上述核心外部入口：`shop-groups`、`publish-create/get`、`price-create/get`、`task-runs`、`notifications`、`runner`、订单/售后/履约同步 runner、发货队列、采购任务、售后列表/责任归因/动作/本地凭证、纠纷同步/本地跟进、订单利润/调整、库存和动销查询。外部系统或后续 agent/skill 优先复用这个脚本，避免各自散落实现 API Key、JSON 和错误处理。

供应商 agent 安全桥同时提供桌面端和命令行两种入口。桌面端“采购任务”页复用 `export_supplier_agent_tasks`、`get_supplier_agent_result_template`、`apply_supplier_agent_results`，支持导出非敏采购任务、生成模板、干跑校验和写回物流/异常/映射结果。采购导出包含外部商品 ID、外部 SKU 和货源链接，便于定位供应商商品，但不得包含收件信息或带登录态的私密链接。命令行 `scripts/wx_xd_supplier_agent.py` 在 `wx_xd_local_api.py` 之上提供同一固定协议：`export`、`template`、`apply`。协议细节见 `docs/supplier-agent-protocol.md`。
