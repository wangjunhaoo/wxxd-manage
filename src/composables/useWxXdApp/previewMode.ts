...
import { defaultAutomationSettings, defaultPublishPricingStrategy, fallbackDeliveryCompanyOptions } from "./constants";
import type {
...
interface PreviewModeDeps {
  ...
  previewAttributeSuggestions: Ref<PublishAttributeSuggestionView[]>;
  ...
  previewProfitAdjustments: Ref<Array<{ order_id: string; kind: string; amount_cents: number }>>;
...
export function createPreviewMode(deps: PreviewModeDeps) {
  ...
  let previewAgentSkills: AgentSkillView[] = [
    {
      ...
      version: "1.0.0",
      description: "审查采集商品，清洗标题、识别不能用于微信小店铺货的图片，并从本地微信类目候选中选择类目。",
      enabled: true,
  ...
  let previewAgentRuns: AgentRunView[] = [
    {
      ...
      model: "preview-model",
      temperature: 0.1,
      input_summary: "采集审查 task_id=collection-preview-1 主图=5 详情图=8 类目候选=3",
  ...
  async function previewCommand<T>(name: string, args?: Record<string, unknown>): Promise<T> {
      ...
      if (name === "import_excel_for_collection") {
        ...
        previewCollectionTasks.value.unshift({
          ...
          collected_data: JSON.stringify({
            ...
            skus: [{ external_sku_id: "sku-1", specs: { 颜色: "默认" }, cost_price: 35, stock: 50 }],
            supplier_name: "淘宝采集",
      ...
      if (name === "retry_collection_task") {
        const task = previewCollectionTasks.value.find((item) => item.id === args?.taskId);
        if (task) {
          task.status = "success";
      ...
      if (name === "run_collection_review_once") {
        ...
        const ids = request?.task_ids || previewCollectionTasks.value.map((task) => task.id);
        let passed = 0;
        previewCollectionTasks.value.forEach((task) => {
          ...
          task.review_result_json = JSON.stringify({ status: "passed", summary: "预览审查通过", issues: [] });
      ...
      if (name === "confirm_collection_review") {
        ...
        const task = previewCollectionTasks.value.find((item) => item.id === request?.task_id);
        if (!task) {
      ...
      if (name === "resume_collection_tasks") {
        return previewCollectionTasks.value.filter((item) => item.status === "pending" || item.status === "running").length as T;
      }
      ...
      if (name === "create_publish_job_from_collection_tasks") {
        ...
        previewCollectionTasks.value.forEach((task) => {
          ...
          task.published_shop_ids = Array.from(new Set([...task.published_shop_ids, ...shopIds]));
          task.publish_job_ids = Array.from(new Set([...task.publish_job_ids, taskId]));
      ...
      if (name === "create_order_price_adjustment_job") {
        ...
        currentOrderPriceAdjustmentJob.value = buildPreviewOrderPriceAdjustmentJob(taskId);
        previewTaskRuns.value.unshift({
          id: taskId,
      ...
      if (name === "get_order_price_adjustment_job") {
        return (currentOrderPriceAdjustmentJob.value || buildPreviewOrderPriceAdjustmentJob(String(args?.taskId || "order_price_preview"))) as T;
      }
      ...
      if (name === "list_agent_runs") {
        ...
        return previewAgentRuns
          .filter((run) => (scene === "all" || run.scene === scene) && (status === "all" || run.status === status))
          .slice(0, limit) as T;
      }
      if (name === "save_agent_skill_settings") {
        ...
        return previewAgentSkills.find((skill) => skill.name === request?.name) as T;
      }
      if (name === "test_agent_skill") {
        ...
        previewAgentSkills = previewAgentSkills.map((skill) => skill.name === skillName
          ? {
              ...skill,
      ...
      if (name === "list_product_management_items") {
        ...
        const allItems = buildPreviewProductSalesAnalysis().map<ProductManagementView>((item) => {
          const managementStatus = item.inventory_risk_status !== "healthy" && item.inventory_risk_status !== "not_listed"
            ? "inventory_risk"
      ...
      if (name === "list_order_management_items") {
        const status = String(args?.status || "all");
        const keyword = String(args?.keyword || "").trim().toLowerCase();
        const allItems = buildPreviewOrderProfitResult("all").items.map<OrderManagementView>((item) => {
          ...
          return {
            ...
            purchase_status: item.purchase_task_count <= 0 ? "missing_purchase_task" : item.missing_cost_count > 0 ? "missing_cost" : "completed",
            shipment_count: item.order_status === "wechat_shipped" ? 1 : 0,
            shipment_status: item.order_status === "wechat_shipped" ? "wechat_shipped" : "none",
            active_aftersale_count: item.order_status === "aftersale_active" ? 1 : 0,
      ...
      if (name === "run_order_price_adjustment_once") {
        const task = previewTaskRuns.value.find((item) => item.task_type === "orders.change_order_price" && ["pending", "queued", "running"].includes(item.status));
        if (task) {
          task.status = "success";
          task.progress = 100;
          task.pending_count = 0;
          ...
          if (currentOrderPriceAdjustmentJob.value?.id === task.id) {
            ...
            currentOrderPriceAdjustmentJob.value.updated_at = "2026-05-22T00:21:00+08:00";
        ...
        if (task) {
          ...
          if (currentPriceJob.value?.id === task.id) {
            ...
            currentPriceJob.value.items.forEach((item) => {
              ...
              item.error_code = null;
              item.error_summary = "本地商品售价校验通过，等待提交微信 updateproduct";
  ...
  function buildPreviewOrderPriceAdjustmentJob(taskId: string): OrderPriceAdjustmentJobView {
      return {
        id: taskId,
